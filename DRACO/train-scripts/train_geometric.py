...
if __name__ == "__main__":
    ...
    #depth_network = SegNet(pretrained=True, withSkipConnections=False)
    ...
    # 2.5 1.5 0.08works fine.
    w_bce = 1.0
    w_photo = 1.0
    w_smooth = 0.11
    w_geometric = 0.1
    print(w_bce, w_photo, w_smooth, w_geometric)
    train_loss = []
    val_loss = []
    train_acc = []
    val_acc = []
    ...
    for epoch in range(config.EPOCHS):
        ...
        for batch, data_sample in enumerate(train_dataloader):
            ...
            loss_4 = criterion_4(data_sample, tr_op).cuda()
            #print(loss_4)
            mvg_loss += loss_2
            geometric_loss += loss_4
            #output = depth_network(data_sample['views'][:, 0])
            ...
            #print(loss_2)
            # 1.5 0.3 0.9
            loss_total = w_bce*loss.to(device) + w_photo*loss_2.to(device) + w_smooth*loss_3.to(device) + w_geometric*loss_4
            #print(loss_total)
    ...
    #torch.save(depth_network.state_dict(), '../checkpoint.pth')
        if (epoch+1)%20==0:
            file_name = f"../check_disp/un_{epoch+1}_net_depth_{w_bce}_{w_photo}_{w_smooth}_{w_geometric}_tgt_mask_geo.pth"
